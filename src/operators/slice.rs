...
fn slice_1(inputs: &[&TensorType], attrs: SliceAttrs) -> BoxResult<TensorType> {
    let data = if let TensorType::F32(data) = inputs[0] {
        data
    } else {
    ...
    Ok(TensorType::F32(common_slice_f32(
        data,
        axes.as_ref(),
        &starts,
        &ends,
...
fn slice_10(inputs: &[&TensorType]) -> BoxResult<TensorType> {
    let data = if let TensorType::F32(data) = inputs[0] {
        data
    } else {
        return Err(anyhow!("Slice1 only support f32"));
    };
    let starts = if let TensorType::I64(starts) = inputs[1] {
        starts
    } else {
        return Err(anyhow!("Slice1 only support i64"));
    };
    let ends = if let TensorType::I64(ends) = inputs[2] {
        ends
    } else {
        return Err(anyhow!("Slice1 only support i64"));
    };
    let axes = match inputs.get(3) {
        Some(TensorType::I64(data)) => Some(data),
        None => None,
        _ => return Err(anyhow!("Slice1 only support i64")),
    };
    let steps = match inputs.get(4) {
        Some(TensorType::I64(steps)) => Some(steps),
        None => None,
        _ => return Err(anyhow!("Slice1 only support i64")),
    };
    Ok(TensorType::F32(common_slice_f32(
